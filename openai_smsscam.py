...
for index, row in tqdm(df[2499:].iterrows(), total=len(df[2499:])):
    i=0
    label = row['v1']
    text = row['v2']
    ...
    with open("data/sms_spam_res.txt", "a+") as f:
        f.write(f"{text},{label},{predict},{tokens}\n")
                
    res = pd.concat([res, pd.DataFrame([[text, label, predict, tokens]], columns=['text', 'truth', 'predict', 'tokens'])],axis=0)
    
res.to_csv("data/sms_spam_res.csv", index=True)