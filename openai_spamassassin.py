...
retry_delay = 30
max_retry = 5
df = pd.read_csv("data/spamassassin.csv",encoding = "ISO-8859-1")
...
for index, row in tqdm(df[:].iterrows(), total=len(df[:])):
    ...
    if type(text) is not str:
        print("Text is None")
        with open("data/openai_spamassassin_res.txt", "a+") as f:
...
res.to_csv("data/openai_spamassassin_res.csv", index=False)